...
%%% PARAMETERS %%
% Read from CSV file
exr_dir = '/media/gluzardo/Data/Stuttgart/bistro/bistro_01/';
tif_dir = '/media/gluzardo/Data/Stuttgart-color-graded/bistro/bistro_01/';
out_dir = '/media/gluzardo/Data/reg_results_itm/bistro/bistro_01/';
border=11;
ft=1;
lt=151;
fj=25;
...
max_val_y=uint16(2^16-1); %max val possible in EXR files
max_val_x=uint8(2^8-1); %max val possible in tif files
channels = 3;
...
for i=ft:fj:lt
      ...
      % Resample to 8 bits
      tif_image=uint8((2^8)*(tif_image/(2^16-1)));
...
bin_size = 16;
r_bin = zeros((max_val_y+1)/bin_size,(max_val_x+1)/bin_size);
g_bin = zeros((max_val_y+1)/bin_size,(max_val_x+1)/bin_size);
b_bin = zeros((max_val_y+1)/bin_size,(max_val_x+1)/bin_size);

for m=1:(max_val_y+1)/bin_size-1
    for n=1:(max_val_x+1)/bin_size-1
